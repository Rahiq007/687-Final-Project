...
try:
    ...
    plt = None
from snake_env_v12 import SnakeEnvV12

class SarsaAgentV12:
    """
    SARSA Agent for SnakeEnvV12 (No Length in State)
    """
    def __init__(self, action_space_size=4, alpha=0.1, gamma=0.99, epsilon=0.2, epsilon_min=0.01, epsilon_decay=0.99995, load_path=None):
        ...
        # Coverage tracking (boolean mask)
        self.visited = np.zeros((8, 256, 3, action_space_size), dtype=bool)
    ...
    def get_coverage(self):
        # Returns percentage of Q-table entries that have been visited/updated
        return 100.0 * np.sum(self.visited) / self.visited.size
        
    def save(self, filename=None):
        if not os.path.exists("lucas_dev/models"):
            os.makedirs("lucas_dev/models")
        if filename is None:
            filename = f"lucas_dev/models/sarsa_v12_{self.total_episodes}_episodes.npz"
        np.savez(filename, 
    ...
    def load(self, filename):
        if os.path.exists(filename):
            ...
            if 'coverage_history' in data:
                self.coverage_history = list(data['coverage_history'])
...
def plot_training_results(agent, window_size=100):
    ...
    plt.title(f'SARSA V12 Training (Episodes: {len(rewards)})')
    plt.ylabel('Reward')
    plt.legend()
    plt.grid(True)
    ...
    # Plot Coverage
    coverage = np.array(agent.coverage_history) if hasattr(agent, 'coverage_history') and agent.coverage_history else []
    if len(coverage) > 0:
        plt.subplot(3, 1, 3)
        plt.plot(coverage, color='purple', label='Q-Table Coverage (%)')
        plt.ylabel('Coverage %')
        plt.xlabel('Episode')
        plt.legend()
        plt.grid(True)
    ...
    plt.savefig(f"lucas_dev/plots/training_plot_v12_{len(rewards)}.png")
    plt.close()
...
def plot_evaluation_results(rewards, lengths, steps):
    ...
    ax.set_title('SARSA V12 Evaluation Results - Snake Length', fontsize=15, fontweight='bold')
    ...
    if not os.path.exists("lucas_dev/plots"): os.makedirs("lucas_dev/plots")
    plt.tight_layout()
    plt.savefig('lucas_dev/plots/sarsa_v12_evaluation_length.png', dpi=150, bbox_inches='tight')
    plt.close()

def distance_check_target(head, action):
    hx, hy = head
    if action == 0: return (hx, hy - 1)
    if action == 1: return (hx, hy + 1)
    if action == 2: return (hx - 1, hy)
    return (hx + 1, hy)

def evaluate_agent(agent, num_episodes=10):
    print("="*70)
    print("EVALUATING TRAINED AGENT (V12)")
    print("="*70)
    
    env = SnakeEnvV12(render_mode='pygame', cell_size=60)
    rewards = []
    lengths = []
    steps_list = []
            ...
            while not done:
                ...
                for y in range(env.grid_size):
                    ...
                    for x in range(env.grid_size):
                        p = (x, y)
                        if p == env.food: char = "F"
                        elif p == env.snake[0]: char = "H"
                        elif p in env.snake: char = "B"
                        elif p in env.obstacles: char = "X"
                        else: char = "."
                        pre_grid_str += f"{char} "
                    pre_grid_str += "\n"
...
def train_sarsa_v12(agent=None, episodes=40000):
    env = SnakeEnvV12(render_mode='text')
    if agent is None: agent = SarsaAgentV12()
    
    print(f"Starting V12 training for {episodes} episodes...")
    start_time = time.time()
    
    start_epsilon = 1.0
    
    # Dynamic Decay Rate Calculation
    # We want the epsilon to decay significantly (e.g., to ~0.02) by 50% of the episodes.
    # Formula: epsilon(t) = min + (max - min) * exp(-decay * t)
    # At t = 0.5 * episodes, we want exp(-decay * t) approx 0.01
    # -decay * (0.5 * episodes) = ln(0.01) approx -4.6
    # decay = 4.6 / (0.5 * episodes)
    decay_rate = 4.6 / (0.5 * episodes)
    
    # Or simpler approximate rule of thumb: decay = 10 / episodes
    # Let's stick to the log-derived one for precision scaling with 'episodes' arg.
    
    for episode in range(episodes):
        # Smooth Exponential Decay relevant to total episodes
        agent.epsilon = agent.epsilon_min + (start_epsilon - agent.epsilon_min) * np.exp(-decay_rate * episode)
        ...
        agent.coverage_history.append(agent.get_coverage())
        
        if (episode + 1) % 1000 == 0:
            avg_reward = sum(agent.reward_history[-100:]) / 100
            avg_length = sum(agent.length_history[-100:]) / 100
            cov = agent.get_coverage()
            print(f"Ep {episode+1}/{episodes} | Total: {agent.total_episodes} | Avg Rew: {avg_reward:.2f} | Avg Len: {avg_length:.2f} | Cov: {cov:.2f}% | Eps: {agent.epsilon:.4f}")
...
if __name__ == "__main__":
    # agent = train_sarsa_v12(episodes=200000)
    agent = SarsaAgentV12(load_path="lucas_dev/models/sarsa_v12_200000_episodes.npz")
