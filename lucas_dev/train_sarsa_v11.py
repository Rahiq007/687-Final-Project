...
try:
    ...
    plt = None
from snake_env_v11 import SnakeEnvV11

class SarsaAgentV11:
    """
    SARSA Agent for SnakeEnvV11
    """
    ...
    def save(self, filename=None):
        ...
        if filename is None:
            filename = f"lucas_dev/models/sarsa_v11_{self.total_episodes}_episodes.npz"
        np.savez(filename, 
    ...
    def load(self, filename):
        if os.path.exists(filename):
            ...
            if 'visited' in data:
                ...
                # If loading old model, infer visited from non-zero Q-values
...
def plot_training_results(agent, window_size=100):
    ...
    plt.title(f'SARSA V11 Training (Episodes: {len(rewards)})')
    plt.ylabel('Reward')
    plt.legend()
    plt.grid(True)
    ...
    plt.savefig(f"lucas_dev/plots/training_plot_v11_{len(rewards)}.png")
    plt.close()
...
def plot_evaluation_results(rewards, lengths, steps):
    ...
    ax.set_title('SARSA V11 Evaluation Results - Snake Length', fontsize=15, fontweight='bold')
    ...
    if not os.path.exists("lucas_dev/plots"): os.makedirs("lucas_dev/plots")
    plt.tight_layout()
    plt.savefig('lucas_dev/plots/sarsa_v11_evaluation_length.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def evaluate_agent(agent, num_episodes=10):
    print("="*70)
    print("EVALUATING TRAINED AGENT (V11)")
    print("="*70)
    
    env = SnakeEnvV11(render_mode='pygame', cell_size=60)
    rewards = []
    lengths = []
    steps_list = []
...
def train_sarsa_v11(agent=None, episodes=40000):
    env = SnakeEnvV11(render_mode='text')
    if agent is None: agent = SarsaAgentV11()
    
    print(f"Starting V11 training for {episodes} episodes...")
...
if __name__ == "__main__":
    # Increased definition of episodes to ensure strong convergence with more complex safety logic
    agent = train_sarsa_v11(episodes=60000)
