...
from snake_env_v3_no_obstacle import SnakeEnvV3NoObstacle
...
class SarsaAgentV3:
    ...
    def save(self, filename=None):
        ...
        if filename is None:
            filename = f"lucas_dev/models/sarsa_v3_no_obstacle_{self.total_episodes}_episodes.npz"
...
def plot_training_results(agent, window_size=100):
    ...
    plt.title(f'SARSA V3 (No Obstacle) Training (Episodes: {len(rewards)})')
    plt.ylabel('Reward')
    plt.legend()
    plt.grid(True)
    ...
    plt.savefig(f"lucas_dev/plots/training_plot_v3_no_obstacle_{len(rewards)}.png")
    plt.close()

def train_sarsa_v3(agent=None, episodes=20000):
    env = SnakeEnvV3NoObstacle(render_mode='text')
    if agent is None: agent = SarsaAgentV3()
    
    print(f"Starting V3 (No Obstacle) training for {episodes} episodes...")
...
def visualize_play(agent):
    print("\nVisualizing V3 (No Obstacle)...")
    env = SnakeEnvV3NoObstacle(render_mode='pygame', cell_size=60)
    state = env.reset()
    env.render()
    time.sleep(1)
    done = False
...
if __name__ == "__main__":
    ...
    # agent = train_sarsa_v3(episodes=200000)
    agent = SarsaAgentV3()
    agent.load("lucas_dev/models/sarsa_v3_no_obstacle_200000_episodes.npz")
