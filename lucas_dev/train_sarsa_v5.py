...
from snake_env_v5 import SnakeEnvV5

class SarsaAgentV5:
    """
    SARSA Agent for SnakeEnvV5
    State Shape: (8, 256, 4) -> (Food, Surround, Length)
    """
    def __init__(self, action_space_size=4, alpha=0.1, gamma=0.99, epsilon=0.2, epsilon_min=0.01, epsilon_decay=0.99995, load_path=None):
        ...
        # State: Food(8), Surround(256), Length(4)
        # Action: 4
        self.q_table = np.zeros((8, 256, 4, action_space_size))
    ...
    def save(self, filename=None):
        ...
        if filename is None:
            filename = f"lucas_dev/models/sarsa_v5_{self.total_episodes}_episodes.npz"
...
def plot_training_results(agent, window_size=100):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(rewards, alpha=0.3, color='purple', label='Raw Reward')
    plt.plot(range(len(rewards)-len(moving_avg_reward), len(rewards)), moving_avg_reward, color='orange', label='Avg Reward')
    plt.title(f'SARSA V5 Training (Episodes: {len(rewards)})')
    plt.ylabel('Reward')
    plt.legend()
    plt.grid(True)

    plt.subplot(2, 1, 2)
    plt.plot(lengths, alpha=0.3, color='cyan', label='Raw Length')
    plt.plot(range(len(lengths)-len(moving_avg_length), len(lengths)), moving_avg_length, color='blue', label='Avg Length')
    ...
    plt.savefig(f"lucas_dev/plots/training_plot_v5_{len(rewards)}.png")
    plt.close()
...
def plot_evaluation_results(rewards, lengths, steps):
    ...
    # Plot reward as bars
    bars = ax.bar(episodes, rewards, color='rebeccapurple', alpha=0.7, label='Episode Reward')
    
    # Add average line
    avg_reward = np.mean(rewards)
    ax.axhline(y=avg_reward, color='gold', linestyle='--', linewidth=2,
    ...
    ax.set_title('SARSA V5 Evaluation Results', 
    ...
    plt.tight_layout()
    plt.savefig('lucas_dev/plots/sarsa_v5_evaluation_results.png', dpi=150, bbox_inches='tight')
    print("✓ Results saved to lucas_dev/plots/sarsa_v5_evaluation_results.png\n")
    plt.close()
...
def evaluate_agent(agent, num_episodes=20):
    ...
    print("="*70)
    print("EVALUATING TRAINED AGENT (V5)")
    print("="*70)
    
    # Use SnakeEnvV5
    env = SnakeEnvV5(render_mode='pygame', cell_size=60)
...
def train_sarsa_v5(agent=None, episodes=20000):
    env = SnakeEnvV5(render_mode='text')
    if agent is None: agent = SarsaAgentV5()
    
    print(f"Starting V5 training for {episodes} episodes...")
...
if __name__ == "__main__":
    ...
    # Increasing to 50k as state space is larger (8192) than V3 (2048)
    agent = train_sarsa_v5(episodes=100000)
